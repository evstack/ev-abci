...
import (
	...
	cosmos "github.com/celestiaorg/tastora/framework/docker/cosmos"
...
func CreateCelestiaChain(ctx context.Context, t *testing.T, dockerClient *client.Client, networkID string) (*cosmos.Chain, error) {
	testEncCfg := testutil.MakeTestEncodingConfig(auth.AppModuleBasic{}, bank.AppModuleBasic{})
	celestia, err := cosmos.NewChainBuilderWithTestName(t, t.Name()).
...
func CreateDANetwork(ctx context.Context, t *testing.T, dockerClient *client.Client, networkID string, celestiaChain *cosmos.Chain) (*da.Network, *da.Node, error) {
	daBuilder := da.NewNetworkBuilderWithTestName(t, t.Name()).
		WithDockerClient(dockerClient).
		WithDockerNetworkID(networkID).
	...
	daNetwork, err := daBuilder.Build(ctx)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to build DA network: %w", err)
	}
	...
	// Use internal hostname for celestia-app node
	networkInfo, err := celestiaChain.GetNodes()[0].GetNetworkInfo(ctx)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get celestia node network info: %w", err)
	}
	celestiaNodeHostname := networkInfo.Internal.Hostname

	err = bridgeNode.Start(ctx,
		da.WithChainID(chainID),
		// my-key is the name of the keyring generated by the bridge node
		da.WithAdditionalStartArguments("--p2p.network", chainID, "--core.ip", celestiaNodeHostname, "--rpc.addr", "0.0.0.0", "--keyring.keyname", "my-key"),
...
func CreateEvolveChain(ctx context.Context, t *testing.T, dockerClient *client.Client, networkID string, bridgeNode *da.Node) (*cosmos.Chain, error) {
	...
	// Compute internal RPC address from network info
	bni, err := bridgeNode.GetNetworkInfo(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to get bridge node network info: %w", err)
	}
	daAddress := fmt.Sprintf("http://%s:%s", bni.Internal.Hostname, bni.Internal.Ports.RPC)
	namespace := generateValidNamespace()

	// bank and auth modules required to deal with bank send tx's
	testEncCfg := testutil.MakeTestEncodingConfig(auth.AppModuleBasic{}, bank.AppModuleBasic{})
	rollkitChain, err := cosmos.NewChainBuilderWithTestName(t, t.Name()).
		WithEncodingConfig(&testEncCfg).
...
func sendFunds(ctx context.Context, chain *cosmos.Chain, fromWallet, toWallet *types.Wallet, amount sdk.Coins) error {
...
func testTransactionSubmissionAndQuery(t *testing.T, ctx context.Context, rollkitChain *cosmos.Chain) {
	// hack to get around global, need to set the address prefix before use.
	...
	// Use chain network info to get GRPC address
	ni, err := rollkitChain.GetNetworkInfo(ctx)
	require.NoError(t, err)
	grpcAddr := "0.0.0.0:" + ni.External.Ports.GRPC
	initialBalance, err := queryBankBalance(ctx, grpcAddr, bobsWallet.GetFormattedAddress(), denom)
	...
	finalBalance, err := queryBankBalance(ctx, grpcAddr, bobsWallet.GetFormattedAddress(), denom)
	...
	carolBalance, err := queryBankBalance(ctx, grpcAddr, carolsWallet.GetFormattedAddress(), denom)
...
// dockerSetupLite is defined in docker_helpers_test.go
